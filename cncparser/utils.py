...
def convert_timedelta(item):
    ...
    Function used to convert timedelta string representation form this:
...
def get_by_date(sequence, date):
    """Returns report object from sequence according to passed date.
...
def _update_default_dict(main, other):
    ...
    keys or just adds key and value if don't
...
def aggregate_data(sequence):
    ...
    ----------
    sequence : iterable
        Sequence that contains report objects.
...
def simplify_job_name(name):
    ...
    -------
    str
        Simplified name of program, means minus path, minus version tags.
